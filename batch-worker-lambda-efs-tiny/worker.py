...
# Use tiny model for Spanish (faster, supports multilingual)
# Options: "tiny" (~5-7x faster), "base" (~3-5x faster), "small" (current)
MODEL_SIZE = os.getenv("MODEL_SIZE", "tiny")
DEVICE = "cpu"
COMPUTE_TYPE = "int8"
CPU_THREADS = 8  # Use more CPU cores
    ...
    if _model_cache is None:
        ...
        try:
            _model_cache = WhisperModel(
                ...
                cpu_threads=CPU_THREADS,  # Use 8 threads for better CPU utilization
...
def transcribe_audio(audio_path: str, language: str = "es") -> dict:
    ...
    try:
        ...
        segments, info = model.transcribe(
            audio_path,
            language=language,
            beam_size=1,  # Faster (default is 5) - reduces quality slightly but much faster
            best_of=1,  # Faster (default is 5)
            word_timestamps=True,
            vad_filter=True,
            vad_parameters=dict(
                min_speech_duration_ms=250,
                min_silence_duration_ms=2000,  # More aggressive VAD for speed
            ),
            condition_on_previous_text=False,  # Faster, doesn't use context from previous segments
        )
