# Batch Worker Lambda with EFS

AWS Lambda function for audio transcription using faster-whisper with EFS-mounted model storage. This approach solves Lambda's 250 MB layer size limit by storing the ~900 MB model on EFS.

## Architecture

- **Runtime**: Python 3.12 (custom container)
- **Model Storage**: AWS EFS (mounted at `/mnt/efs/models`)
- **Model**: faster-whisper "small" (~900 MB)
